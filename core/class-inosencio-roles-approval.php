...
class Inosencio_User_Roles_Approval {
	...
	function __construct( $restricted_user_roles, $restricted_content_types = array() ) {
		...
		if ( isset( $_GET['inosencio_revision_delete'] ) ) {
	...
	public function modify_pre_get_posts( $wp_query ) {
        ...
        $user_role = inosencio_userroles_current_role();
	...
	public function modify_meta_boxes() {
		...
		$current_user_role = inosencio_userroles_current_role();

		if ( in_array( $current_user_role, $this->restricted_user_roles ) ||
			( current_user_can( 'manage_options' ) && get_post_meta( get_the_ID(), 'inosencio_revision_parent', true ) )
		   ) {
			...
			if ( in_array( $post_type, $this->restricted_content_types ) ) {
				...
				add_meta_box(
					'inosencio_section_manager_submit',
					'Submit',
	...
	public function save_revision_mb() {
		...
		wp_nonce_field( 'inosencio_submit_review', 'inosencio_submit_review_nonce' );

		if ( $revision_parent = get_post_meta( get_the_ID(), 'inosencio_revision_parent', true ) ) :

			$delete_revision_link = add_query_arg(
				'inosencio_revision_delete',
				$revision_parent,
	...
	public function push_revision_mb() {
		...
		$delete_revision_link = add_query_arg(
			'inosencio_revision_delete',
			get_post_meta( get_the_ID(), 'inosencio_revision_parent', true ),
			get_delete_post_link( get_the_ID(), '', true )
		);
		?>
	...
	public function revision_notice() {
		...
		if ( $parent_ID = get_post_meta( get_the_ID(), 'inosencio_revision_parent', true ) ) : ?>
			...
			<div class="notice notice-warning">
				<p>
					...
					<a href="<?php echo add_query_arg( 'inosencio_view_original', '1', get_edit_post_link( $parent_ID ) ); ?>"
					   class="button">
						View Original
					</a>
					<?php endif; ?>
		...
		if ( $revision_ID = get_post_meta( get_the_ID(), 'inosencio_revision_child', true ) ) : ?>
	...
	function revision_redirect() {
		...
		if ( ! is_admin() ||
			! ( $revision_ID = get_post_meta( get_the_ID(), 'inosencio_revision_child', true ) ) ||
			isset( $_GET['inosencio_view_original'] ) ||
	...
	public function save_pending( $post_ID, $data ) {
		...
		if ( ! isset( $_POST['inosencio_submit_review_nonce'] ) ||
			! wp_verify_nonce( $_POST['inosencio_submit_review_nonce'], 'inosencio_submit_review' ) ||
		...
		if ( $revision_parent = get_post_meta( $post_ID, 'inosencio_revision_parent', true ) ) {
		...
		if ( $submit ) {
			...
			wp_mail( get_option( 'admin_email' ), 'Inosencio - New Revision Submission', $message );
		...
		if ( $revision_ID !== $post_ID && $revision_ID && ! is_wp_error( $revision_ID ) ) {
			...
			update_post_meta( $revision_ID, 'inosencio_revision_parent', $post_ID );
			update_post_meta( $post_ID, 'inosencio_revision_child', $revision_ID );
	...
	public function push_revision( $post_ID, $data ) {
		...
		if ( ! current_user_can( 'manage_options' ) ||
			! ( $parent_ID = get_post_meta( $post_ID, 'inosencio_revision_parent', true ) )
		   ) {
			return;
		}
		...
		delete_post_meta( $parent_ID, 'inosencio_revision_child' );
	...
	public function list_table_title( $title ) {
		...
		if ( $child_ID = get_post_meta( get_the_ID(), 'inosencio_revision_child', true ) ) {
		...
		elseif ( get_post_meta( get_the_ID(), 'inosencio_revision_parent', true ) ) {
	...
	function delete_revision_redirect() {
		...
		$parent_ID = $_GET['inosencio_revision_delete'];
	...
	private function preserve_section_meta( $post_ID, $revision_ID ) {
		 ...
		 * @since 1.0.0
		 */
		$exclude_meta = apply_filters( 'inosencio_userrole_exclude_revision_meta_keys', array(
			'_edit_last',
			'_edit_lock',
		) );
